...
export const createShortUrl = async (req, res, next) => {
    try {
        ...
        res.status(201).json(new ApiResponse(201, { url: newUrl }, 'URL shortened successfully'));
    } catch (error) {
        next(error);
    }
};
...
export const getUserUrls = async (req, res, next) => {
    try {
        ...
        res.status(200).json(new ApiResponse(200, { urls, results: urls.length }));
    } catch (error) {
        next(error);
    }
};
...
export const deleteShortUrl = async (req, res, next) => {
    try {
        ...
        res.status(204).json(new ApiResponse(204, null, 'URL deleted'));
    } catch (error) {
        next(error);
    }
};
...
export const disableShortUrl = async (req, res, next) => {
    try {
        ...
        res.status(200).json(new ApiResponse(200, { url: updatedUrl }, 'URL disabled'));
    } catch (error) {
        next(error);
    }
};
...
export const enableShortUrl = async (req, res, next) => {
    try {
        ...
        res.status(200).json(new ApiResponse(200, { url: updatedUrl }, 'URL enabled'));
    } catch (error) {
        next(error);
    }
};